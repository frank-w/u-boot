...
static uint32_t msdc_fifo_tx_bytes(struct msdc_host *host)
{
	return (mmio_read_32((uintptr_t)&host->base->msdc_fifocs) &
...
static int msdc_card_busy(void)
{
	struct msdc_host *host = &_host;
	uint32_t status = mmio_read_32((uintptr_t)&host->base->msdc_ps);
...
static int mtk_mmc_prepare(int lba, uintptr_t buf, size_t size, int write)
{
	xfer_size = size;
	xfer_write = !!write;
...
static void msdc_fifo_write(struct msdc_host *host, const uint8_t *buf,
			    size_t size)
{
	const uint32_t *wbuf;

	while ((size_t)buf % 4) {
		mmio_write_8((uintptr_t)&host->base->msdc_txdata, *buf++);
		size--;
	}

	wbuf = (const uint32_t *)buf;
	while (size >= 4) {
		mmio_write_32((uintptr_t)&host->base->msdc_txdata, *wbuf++);
		size -= 4;
	}
...
static int mtk_mmc_write(int lba, uintptr_t buf, size_t size)
{
	...
	while (1) {
		...
		if ((status & MSDC_INT_XFER_COMPL) || !size) {
			if (size) {
				ERROR("MSDC: Data is not fully written\n");
				ret = -EIO;
			}
...
static void msdc_init_hw(void)
{
	...
	if (host->dev_comp->async_fifo) {
		...
		if (host->dev_comp->async_fifo_crcsts)
			mmio_setbits_32((uintptr_t)&host->base->patch_bit2,
					MSDC_PB2_CFGCRCSTS);
		else
			mmio_clrbits_32((uintptr_t)&host->base->patch_bit2,
					MSDC_PB2_CFGCRCSTS);
...
{
	return mtk_mmc_device_info.device_size;
}

uint32_t mtk_mmc_block_count(void)
{
	return (uint32_t)(mtk_mmc_device_info.device_size /
			  mtk_mmc_device_info.block_size);
}

enum mmc_device_type mtk_mmc_device_type(void)
{
	return mtk_mmc_device_info.mmc_dev_type;
}
