...
static int spi_nand_read_casn(struct casn_page *casn, uint8_t *sel)
{
	...
	for(i=0; i<sizeof(casn_offset)/sizeof(uint8_t); i++) {
		...
		valid = spi_nand_check_casn(casn, sel);
		if (!valid) {
			break;
		}
	}
...
static int spi_nand_read_pp(struct parameter_page *pp, uint8_t *sel)
{
	...
	for(i=0; i<sizeof(pp_offset)/sizeof(uint8_t); i++) {
		ret = spi_nand_load_page(pp_offset[i]);
		if (ret != 0) {
			goto restore;
		}
		...
		ret = spi_nand_read_from_cache(0x0, 0x0, (uint8_t *)pp,
					       PARAMETER_PAGE_LENGTH * PARAMETER_PAGE_COPIES);
		if (ret != 0) {
			goto restore;
		}

		valid = spi_nand_check_pp(pp, sel);
		if (!valid) {
			break;
		}
	}
				    ...
				    uint8_t *vendor_id)
{
	...
	if (nand_info) {
		...
		device->nand_dev->oob_size = nand_info->memorg.sparesize;
