...
void bl31_mbedtls_init(void)
{
	...
	if (ret) {
		VERBOSE("MBEDTLS: Failed to get heap\n");
		panic();
	}
			 ...
			 uint8_t *out, size_t out_len)
{
	...
	if (!md_info) {
		VERBOSE("MBEDTLS: Failed to get MD info\n");
	...
	if (ret)
		VERBOSE("MBEDTLS: HKDF key derivation failed: %d\n", ret);
		       ...
		       const uint8_t padding_mode)
{
	...
	ret = mbedtls_cipher_set_padding_mode(&aes_ctx, padding_mode);
	if (ret) {
	...
	ret = mbedtls_cipher_finish(&aes_ctx, out + len, &len);
	if (ret)
			 ...
			 uint8_t *out)
{
	...
	ret = mbedtls_gcm_setkey(&gcm_ctx, MBEDTLS_CIPHER_ID_AES,
				 key, (key_len << 3));
	if (ret) {
		VERBOSE("MBEDTLS: Failed to set GCM key: %d\n", ret);
	...
	ret = mbedtls_gcm_auth_decrypt(&gcm_ctx, cipher_len, iv, iv_len,
				       ...
				       cipher, out);
	if (ret)
		VERBOSE("MBEDTLS: GCM Decryption failed: %d\n", ret);
